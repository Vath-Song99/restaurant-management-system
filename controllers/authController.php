...
class AuthController
{
    ...
    public function loginWithQrcode($token = null)
    {

        if (!$token) {
            FlashHelper::setFlash('login_error', 'Invalid QR code', 'alert alert-danger');
        ...
        if ($userRole->role_name == 'admin' || $userRole->role_name == 'staff' || $userRole->role_name == 'manager') {
            header('Location: ' . BASE_URL . '/dashboard');
            return;
        }
        header('Location: ' . BASE_URL . '/');
